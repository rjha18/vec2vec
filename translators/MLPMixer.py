...
class EmbPatches(nn.Module):
    def __init__(self, input_dim: int, patch_size: int, hidden_size: int):
        super().__init__()
        assert hidden_size % patch_size == 0
        self.patch_weights = nn.Parameter(torch.randn(patch_size, dim // patch_size))
        self.projection = nn.Sequential(
            nn.ReLU(),
            nn.Dropout(0.1),   
            nn.Linear(dim // patch_size, hidden_size)
        )

    def forward(self, emb: torch.Tensor) -> torch.Tensor:
        patches = emb.reshape(emb.shape[0], emb.shape[1] // self.patch_size, self.patch_size)
        patches = patches @ self.patch_weights
        patches = self.projection(patches)
        return patches

class MlpBlock(nn.Module):
    def __init__(self, in_dim: int, hidden_dim: int, p=0.1):
        super().__init__()
...
class MLPMixer(nn.Module):
    def __init__(
        ...
        self.patch = EmbPatches(input_channel=in_dim, dim=hidden_dim)
        self.mixer_blocks = nn.Sequential([
            MixerBlock(num_patches=num_patches, chn_dim=1, tok_hid_dim=hidden_dim, chn_hid_dim=hidden_dim)
    ...
    def forward(self, x):
        ...
        return x + self.out(x.mean(dim=1))
